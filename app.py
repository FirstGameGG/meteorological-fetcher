...
def validate_date_range(start_date: date, end_date: date):
    if start_date > end_date:
        st.error(":material/error: วันที่เริ่มต้นต้องไม่มากกว่าวันที่สิ้นสุด")
        return False, end_date

    today = date.today()
    if start_date > today:
        st.error(":material/error: วันที่เริ่มต้นต้องไม่เป็นวันในอนาคต")
        return False, end_date
...
# --- User inputs ---
with st.container():
    st.markdown("### :material/settings: กำหนดภูมิภาคและช่วงเวลาการสืบค้นข้อมูล")
    selected_regions = render_region_selector(REGION_COLORS)

    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input(
            ":material/calendar_month: วันที่เริ่มต้น",
            value=date(2005, 1, 1),
            min_value=date(1950, 1, 1),
            max_value=date.today(),
        )
    with col2:
        end_date = st.date_input(":material/calendar_month: วันที่สิ้นสุด", date.today())

wmo_stations = {k: v for k, v in all_stations.items() if v["region"] in selected_regions}

# --- Execute processing ---
if st.button(":material/rocket_launch: ประมวลผลและทำความสะอาดข้อมูล", type="primary", use_container_width=True):
    if not selected_regions:
        st.error(":material/error: กรุณาเลือกข้อมูลอย่างน้อย 1 ภูมิภาค")
        st.stop()

    if not wmo_stations:
        st.error(":material/error: ไม่พบสถานีอุตุนิยมวิทยาในภูมิภาคที่เลือก")
        st.stop()
    ...
    if all_weather_data:
        try:
            ...
            progress_bar.empty()
            status_text.empty()
            st.error(f":material/error: เกิดข้อผิดพลาดระหว่างเตรียมข้อมูลสภาพอากาศ: {e}")
            st.stop()
        ...
        status_text.markdown(":material/check_circle: เสร็จสิ้นกระบวนการ!")
        progress_bar.empty()
        ...
        st.markdown("### :material/map: แผนที่แสดงพิกัดสถานีอุตุนิยมวิทยาที่สมบูรณ์")
        if fetched_stations:
        ...
        st.markdown("### :material/show_chart: สถิติภูมิอากาศและดัชนี ONI รายเดือน")
        try:
        ...
        st.markdown("### :material/bar_chart: ตารางสรุปผลข้อมูลรายเดือน")
        st.dataframe(df_monthly, use_container_width=True)

        st.markdown("### :material/menu_book: พจนานุกรมข้อมูล (Data Dictionary)")
        ...
        st.markdown("### :material/download: ส่งออกข้อมูล (รวมดัชนี ONI)")
        ...
        progress_bar.empty()
        status_text.empty()
        st.error(":material/error: ไม่พบข้อมูลสภาพอากาศในช่วงเวลาที่ระบุ หรือล้มเหลวในการเชื่อมต่อกับทุกสถานี")
        if failed_stations:
            preview = pd.DataFrame(failed_stations)
            st.markdown("### :material/search: ตัวอย่างรายละเอียดข้อผิดพลาด (10 รายการแรก)")
